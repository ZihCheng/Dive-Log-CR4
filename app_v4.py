...
async def sync_from_watch(status_placeholder):
    ...
    try:
        async with BleakClient(TARGET_ADDRESS) as client:
            status_placeholder.info("🔗 藍牙已連線！正在搜尋手錶日誌...")
            ...
            while True:
                ...
                if doc_id in existing_ids: dive_index += 1; continue
                
                status_placeholder.warning(f"🔍 檢查第 {dive_index} 潛...")
...
def load_all_data_from_cloud():
    ...
    for doc in docs:
        ...
        # 🚀 修正點：確保 header_hex 也被存在 all_logs 裡面
        all_logs.append({
            "filename": doc.id, 
            "header_hex": data["header_hex"], 
            **info, 
            "profile_hex_list": data["profile_hex_list"]
        })
...
if sync_btn:
    with msg_cont:
        ...
        if ok: st.session_state.sync_msg = f"✅ 完成！同步了 {res} 筆新紀錄。"; st.cache_data.clear(); st.rerun()
